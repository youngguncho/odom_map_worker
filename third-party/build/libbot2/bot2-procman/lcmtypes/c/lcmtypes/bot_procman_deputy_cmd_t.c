...
static int __bot_procman_deputy_cmd_t_hash_computed;
static int64_t __bot_procman_deputy_cmd_t_hash;

int64_t __bot_procman_deputy_cmd_t_hash_recursive(const __lcm_hash_ptr *p)
{
    const __lcm_hash_ptr *fp;
    for (fp = p; fp != NULL; fp = fp->parent)
        if (fp->v == __bot_procman_deputy_cmd_t_get_hash)
            return 0;

    __lcm_hash_ptr cp;
    cp.parent =  p;
    cp.v = (void*)__bot_procman_deputy_cmd_t_get_hash;
    (void) cp;

    int64_t hash = (int64_t)0xed5fbe5982ac8353LL
...
{
    if (!__bot_procman_deputy_cmd_t_hash_computed) {
        __bot_procman_deputy_cmd_t_hash = __bot_procman_deputy_cmd_t_hash_recursive(NULL);
        __bot_procman_deputy_cmd_t_hash_computed = 1;
    }

    return __bot_procman_deputy_cmd_t_hash;
}
...
{
    int pos = 0, thislen;
    int64_t hash = __bot_procman_deputy_cmd_t_get_hash();

    thislen = __int64_t_encode_array(buf, offset + pos, maxlen - pos, &hash, 1);
    if (thislen < 0) return thislen; else pos += thislen;

    thislen = __bot_procman_deputy_cmd_t_encode_array(buf, offset + pos, maxlen - pos, p, 1);
    if (thislen < 0) return thislen; else pos += thislen;
...
{
    return 8 + __bot_procman_deputy_cmd_t_encoded_array_size(p, 1);
}
...
int bot_procman_deputy_cmd_t_decode(const void *buf, int offset, int maxlen, bot_procman_deputy_cmd_t *p)
{
    int pos = 0, thislen;
    int64_t hash = __bot_procman_deputy_cmd_t_get_hash();
    ...
    thislen = __bot_procman_deputy_cmd_t_decode_array(buf, offset + pos, maxlen - pos, p, 1);
    if (thislen < 0) return thislen; else pos += thislen;
...
{
    return __bot_procman_deputy_cmd_t_decode_array_cleanup(p, 1);
}
...
bot_procman_deputy_cmd_t *bot_procman_deputy_cmd_t_copy(const bot_procman_deputy_cmd_t *p)
{
    bot_procman_deputy_cmd_t *q = (bot_procman_deputy_cmd_t*) malloc(sizeof(bot_procman_deputy_cmd_t));
    __bot_procman_deputy_cmd_t_clone_array(p, q, 1);
    return q;
}

void bot_procman_deputy_cmd_t_destroy(bot_procman_deputy_cmd_t *p)
{
    __bot_procman_deputy_cmd_t_decode_array_cleanup(p, 1);
    free(p);
}

int bot_procman_deputy_cmd_t_publish(lcm_t *lc, const char *channel, const bot_procman_deputy_cmd_t *p)
{
      int max_data_size = bot_procman_deputy_cmd_t_encoded_size (p);
      uint8_t *buf = (uint8_t*) malloc (max_data_size);
      if (!buf) return -1;
      int data_size = bot_procman_deputy_cmd_t_encode (buf, 0, max_data_size, p);
      if (data_size < 0) {
          free (buf);
          return data_size;
      }
...
struct _bot_procman_deputy_cmd_t_subscription_t {
    bot_procman_deputy_cmd_t_handler_t user_handler;
    void *userdata;
    lcm_subscription_t *lc_h;
};
static
void bot_procman_deputy_cmd_t_handler_stub (const lcm_recv_buf_t *rbuf,
                            const char *channel, void *userdata)
{
    int status;
    bot_procman_deputy_cmd_t p;
    memset(&p, 0, sizeof(bot_procman_deputy_cmd_t));
    status = bot_procman_deputy_cmd_t_decode (rbuf->data, 0, rbuf->data_size, &p);
    if (status < 0) {
        fprintf (stderr, "error %d decoding bot_procman_deputy_cmd_t!!!\n", status);
        return;
    }

    bot_procman_deputy_cmd_t_subscription_t *h = (bot_procman_deputy_cmd_t_subscription_t*) userdata;
    h->user_handler (rbuf, channel, &p, h->userdata);

    bot_procman_deputy_cmd_t_decode_cleanup (&p);
}

bot_procman_deputy_cmd_t_subscription_t* bot_procman_deputy_cmd_t_subscribe (lcm_t *lcm,
                    const char *channel,
                    bot_procman_deputy_cmd_t_handler_t f, void *userdata)
{
    bot_procman_deputy_cmd_t_subscription_t *n = (bot_procman_deputy_cmd_t_subscription_t*)
                       malloc(sizeof(bot_procman_deputy_cmd_t_subscription_t));
    n->user_handler = f;
    n->userdata = userdata;
    n->lc_h = lcm_subscribe (lcm, channel,
                                 bot_procman_deputy_cmd_t_handler_stub, n);
    if (n->lc_h == NULL) {
        fprintf (stderr,"couldn't reg bot_procman_deputy_cmd_t LCM handler!\n");
        free (n);
        return NULL;
    }
    return n;
...
int bot_procman_deputy_cmd_t_unsubscribe(lcm_t *lcm, bot_procman_deputy_cmd_t_subscription_t* hid)
{
    ...
    if (0 != status) {
        fprintf(stderr,
           "couldn't unsubscribe bot_procman_deputy_cmd_t_handler %p!\n", hid);
        return -1;
    }
    free (hid);
    return 0;
